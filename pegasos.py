...
def objective_function(X, y, w, lamb):
    ...
    #N = X.shape[0]
    #obj_value = 0.5 * lamb * np.sqrt(np.dot(w.T, w)) + (1/N) * np.sum(np.max(np.hstack((np.zeros((N,1)), 1 - np.multiply(y,np.dot(X,w)))), axis=1))
    #return obj_value[0][0]

    #return obj_value
    N = X.shape[0]
    obj_value = 0.5*lamb*(np.sum(w**2)) + 1/N*np.sum(np.maximum(np.zeros((1,N)),np.ones((1,N))-np.multiply(np.expand_dims(y,axis=0),np.dot(w.T, X.T))))
    return obj_value
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        # you need to fill in your solution here
        temp = np.multiply(ytrain[A_t].reshape((k,1)),np.dot(Xtrain[A_t],w))<1
        A_t = np.multiply(A_t.reshape((k,1)), temp)  
        A_t = A_t[np.nonzero(A_t)]      
        step_size = 1/(lamb*iter)
        w_half = (1-step_size*lamb)*w + (step_size/k)*np.sum(np.multiply(ytrain[A_t].reshape((A_t.shape[0],1)),Xtrain[A_t]),axis=0).reshape((D,1))
        w = np.minimum(1,(1/np.sqrt(lamb))/np.sqrt(np.sum(w_half**2)))*w_half
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    test_acc = np.mean((2*(np.dot(w_l.T, Xtest.T) > t)-1) == ytest)
    return test_acc
